...
<div class='w-full h-full bg-slate-300 dark:bg-slate-900 flex'>
  <div class='w-60 bg-slate-100 dark:bg-slate-700 relative h-full'>
    <div class='absolute inset-0 margin-0 overflow-y-auto scrollbar-hidden'>
      <div class='flex flex-col justify-start items-start'>
        {#each $noteManager as noteName, i}
          <button on:click={() => {selectedNote = null; setTimeout(() => {selectedNote = noteName;})}} class='p-1 truncate px-2 font-semibold dark:text-slate-300'>
            {noteName}.txt
          </button>
        {/each}
      </div>
